...
async def get_contacts(name: str | None, surname: str | None, email: str | None, limit: int, offset: int, db: AsyncSession):
...
async def get_upcoming_birthdays(days_range: int, db: AsyncSession):
    today = datetime.today().date()
    start_period = today.strftime('%m-%d')
    end_period = (today + timedelta(days_range)).strftime('%m-%d')
    statement = select(Contact).filter(func.to_char(Contact.birthday, 'MM-DD').between(start_period, end_period))
    contacts = await db.execute(statement)
    return contacts.scalars().all()
